...
(function ($) {
    ...
    var sbdstoredNoticeId = localStorage.getItem('qcld_hero_Notice_set');
    var qcld_hero_Notice_time_set = localStorage.getItem('qcld_hero_Notice_time_set');

    var notice_current_time = Math.round(new Date().getTime() / 1000);

    if ('hero-msg' == sbdstoredNoticeId && qcld_hero_Notice_time_set > notice_current_time  ) {
        $('#message-hero').css({'display': 'none'});
    }

    $(document).on('click', '#message-hero .notice-dismiss', function(e){

        var currentDom = $(this);
        var currentWrap = currentDom.closest('.notice');
        currentWrap.css({'display': 'none'});
        localStorage.setItem('qcld_hero_Notice_set', 'hero-msg');
        var ts = Math.round(new Date().getTime() / 1000);
        var tsYesterday = ts + (24 * 3600);
        localStorage.setItem('qcld_hero_Notice_time_set', tsYesterday);
