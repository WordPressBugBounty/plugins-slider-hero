...
if( !class_exists('QcPluginUpgradeToProNotice') )
{
	class QcPluginUpgradeToProNotice {
		...
		public $link_text = "Upgrade to Pro (Now with Monetize Option)";
		public $link_class = "";
		...
		function func_show_upgrade_link_with_action_links( $links )
		{
			$links = array_merge( $links, array(
				'<a title="'.esc_attr($this->link_text).'" class="'.esc_attr($this->link_class).'" style="font-weight: bold; color: '.esc_attr($this->link_color).';" href="' . esc_url( $this->upgrade_link ) . '" target="'.esc_attr( $this->link_target ).'">' . esc_html( $this->link_text) . '</a>'
			) );
...
$instance_slider_hero_free = new QcPluginUpgradeToProNotice();
...
{ 
	...
	$instance_slider_hero_free->upgrade_link = "https://www.quantumcloud.com/products/slider-hero/";
	//$instance_sldf2->link_color = "#FCB214";
	$instance_slider_hero_free->link_text = "Upgrade To Pro";
	//$instance_sldf2->link_class = "";
	//$instance_sldf2->link_target = "_blank";

	$instance_slider_hero_free->plugin_slug = 'slider-hero'; //Plugin Slug. i.e. Folder Name
	$instance_slider_hero_free->plugin_main_file = 'qcld-slider-main.php'; //Primary file of the pluign
	$instance_slider_hero_free->plugin_menu_slug = 'Slider-Hero'; //Main Menu Slug

	$instance_slider_hero_free->show_with_action_links = true; //show in plugin.php page
	$instance_slider_hero_free->show_with_meta_links = true; //show in plugin.php page
	$instance_slider_hero_free->show_with_plugin_menu = true; //show inside parent menu of the plugin

	if( $instance_slider_hero_free->check_if_plugin_page() )
	{
		$instance_slider_hero_free->hook_with_plugin_action_links();
		$instance_slider_hero_free->hook_with_plugin_meta_links();
	}
